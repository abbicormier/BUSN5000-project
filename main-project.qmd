...
![Project-course alignment](assets/instructions_img_1.png){fig-alt="Diagram showing how the 35 project slides map to weeks 1-7 of the course: Week 1 Data Fundamentals → Slides 1-7; Week 2 Beginning to Learn → Slides 8-12; Week 3 Models for Exploration → Slides 13-20; Week 4 Making Inferences → Slides 21-26; Week 5 Sources of Bias → Through Slide 20 (optional check); Week 7 Regression Fundamentals → Slides 27-35"}
...
- `Project_Progress_Check_Tables_and_Figures.pdf` — reference for
  the progress check (Week 4)
...
each slide by default. A handful of chunks individually set
`echo = FALSE` (the `.5` chunks that display tables/figures, the
three `datasummary` chunks, and the appendix `var_doc_table`).
...
exception of the final model "Only Singles", which estimates the
same relationship as the "Add Person" model but on the new
subset.
:::
