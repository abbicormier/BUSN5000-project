...
<!--
  ...
  `project_progress_check_template.html` was also removed —
  redundant with the Final Project reference in the Submission
  chapter. `cpsmar24_documentation.pdf` was moved to the
  pre-project pack (paired with the data).
-->
- `check_setup_project.R` — verifies your file sort for the main
  project

Here's the destination for each new file:
...
Then run `check_setup_project.R` from RStudio (open the script,
Select All, Run) to confirm everything's in place.

## Optional bonus midway through

Below is the slide-by-slide walkthrough for the entire project,
to be submitted in full at the end of the semester. You don't
have to wait until then to show progress — partway through, the
optional **Project Progress Check** lets you submit an
in-progress version of your deck for up to 5 bonus points (and
serves as a safety net if your pre-project lost points).

See the [Progress Check chapter](progress-check.qmd) for what's
scored and when it's due.
...
Stay disciplined — concise writing is what's being scored, not
volume.
...
The point allocation for each scored slide is shown in parentheses
