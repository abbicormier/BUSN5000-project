...
**Cause:** `project_slidedeck.css` is anywhere other than
`Project/css/`.

**Fix:** Move the file into `Project/css/`. Re-knit. Visual example
...
**Fix:** Restore the YAML to its original values, leaving only the
`author:` line as your edit. If unsure, open a fresh copy of
`pre_project.Rmd` or `project.Rmd` and copy the YAML block over.
...
**Fix:** Verify the CSS file is in `Project/css/`. Re-knit.
...
**Fix:** Move CSS to `Project/css/`, re-knit, then save with
...
specifies `ioslides_presentation` for a reason.
...
- **For content-related questions**, contact **TAL staff** first.
  If TAL staff can't solve the problem, then email Abbi or Chris.
- **For administrative questions**, email **Abbi first**. She'll
  loop in Chris if needed.
- **One thread per issue.** Don't send the same problem to both
  Abbi and Chris on separate emails. We will loop the other in if
  needed. Duplicate emails waste resources for your classmates.
